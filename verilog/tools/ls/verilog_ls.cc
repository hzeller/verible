...
using verilog::CreateDiagnostics;
using verilog::CreateDocumentSymbolOutline;
using verilog::GenerateLinterCodeActions;

// Kate can't display all the types we'd like to display in the tree, so
// we do some mapping for now (e.g. 'namespace' -> 'class').
// Fix this by sending a patch or bug to kate.
...
static void SendDiagnostics(const std::string &uri,
  ...
  params.uri = uri;
  params.diagnostics =
      verilog::CreateDiagnostics(buffer->parser(), buffer->lint_result());
      ...
      [&parsed_buffers](const DocumentSymbolParams &p) -> nlohmann::json {
        ...
        if (analyzed) {
          return CreateDocumentSymbolOutline(
              analyzed->parser(), absl::GetFlag(FLAGS_kate_workaround), p);
        }
